...
MOVECLIP_T:  entities never clip against themselves, or their owner
...
LINK_T FUNCTIONS (link_t is defined in common.h)
// Baker: Moved these functions from common.c to here (only usage)
...
void InsertLinkAfter (link_t *l, link_t *after) // Function unused in code
{
	l->next = after->next;
	l->prev = after;
	l->prev->next = l;
...
// decide which clipping hull to use, based on the size
	...
	{	// explicit hulls in the BSP model
		...
		if (!model || model->type != mod_brush)
			Host_Error ("SOLID_BSP with a non bsp model"); // was MOVETYPE_PUSH with a non bsp model
		...
		{
			if (model->hulls[3].available && size[2] < 54) // pick the nearest of 36 or 72
...
void SV_UnlinkEdict (edict_t *ent)
{
	...
	// Baker: White room fix.  At least part of it ...
...
// touch linked edicts
	...
	sv_link_next = &next; // Baker: part of whiteroom fix
	...
	for (l = node->trigger_edicts.next ; l != &node->trigger_edicts ; l = next)
	{
		...
		// No telefrag for you
		if (sv.coop_physics_clientnum /* means protected */ ) {
...
*/
	...
	sv_link_next = NULL; // Baker: Part of white room fix.
...
// add an efrag if the node is a leaf
	...
	if ( node->contents < 0)
	{
// Baker: According to Spike and MH, this can go crazy and a large entity can be in countless leafs
//        It can become invisible or start to flicker if it overflows the leafs because it will fail visibility tests
//        When the unmarked leafs aren't checked.  Rubicon Rumble has a large entity in 20,000 leafs, they say.
//        The Spike/MH solution is to trivially accept an overflowed entity, adjusting upwards the max to 32.
...
===============
*/
void SV_LinkEdict (edict_t *ent, cbool touch_triggers)
{
	areanode_t	*node;
...
// set the abs box
// ROTATE START
	if (SUPPORTS_ROTATION(sv.protocol) && ent->v.solid == SOLID_BSP &&
   (ent->v.angles[0] || ent->v.angles[1] || ent->v.angles[2]) )
   {   // expand for rotation
		float      vmax, v;
		int         i;

		vmax = DotProduct(ent->v.mins, ent->v.mins);
		...
		for (i=0 ; i<3 ; i++)
		{
			ent->v.absmin[i] = ent->v.origin[i] - vmax;
			ent->v.absmax[i] = ent->v.origin[i] + vmax;
		}
	}
   else
// ROTATE END
...
====================
*/
// Called exclusively by SV_Move
static void SV_Move_ClipToLinks ( areanode_t *node, moveclip_t *clip )
{
	link_t		*l, *next;
	edict_t		*touch;
	trace_t		trace;
...
// touch linked edicts
	for (l = node->solid_edicts.next ; l != &node->solid_edicts ; l = next)
	{
		...
		if (sv.coop_physics_clientnum /* means protected */ ) {
			// COOP PROTECTION AGAINST TELEFRAG:  IGNORE SLIDEBOX (other players)
			if (touch->v.solid == SOLID_SLIDEBOX)
				continue;	// We don't physically interact with other players.
			if (touch->v.solid == SOLID_TRIGGER) {
				if (touch->v.movetype == MOVETYPE_NONE) {
					const char *s = PR_GetString(touch->v.classname);
					if (!strcmp(PR_GetString(touch->v.classname), "teledeath") )
						continue;  // We don't physically interact with telefrag
				}
...
#if 1 // SOLID CORPSE EXTENSION
		{
			//corpses are nonsolid to slidebox.  Note: Monsters can't seem to target solid corpse with ranged attacks, if a player uses movetype solid corpse.
...
// recurse down both sides
	...
	if ( clip->boxmaxs[node->axis] > node->dist )
		SV_Move_ClipToLinks ( node->children[0], clip );
	if ( clip->boxmins[node->axis] < node->dist )
		SV_Move_ClipToLinks ( node->children[1], clip );
}
...
==================
*/
// Baker: Called all over the place
...
// clip to entities
	SV_Move_ClipToLinks ( sv_areanodes, &clip );
