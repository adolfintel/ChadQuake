...
#include "quakedef.h" // Baker: Modifications - exclusively skybox and r_wateralpha


static int	miplevel;

float		scale_for_mip;
int		screenwidth /* qasm */;
int		ubasestep /* qasm */, errorterm /* qasm */, erroradjustup /* qasm */, erroradjustdown /* qasm */;
...
// TODO: could preset a lot of this at mode set time
	...
	else
	{
		for (s = &surfaces[1] ; s<surface_p ; s++)
		{
			...
			{
				if (s->flags & SURF_DRAWLAVA) winquake_surface_liquid_alpha = frame.lavaalpha;
				else if (s->flags & SURF_DRAWSLIME) winquake_surface_liquid_alpha = frame.slimealpha;
				else if (s->flags & SURF_DRAWWATER) winquake_surface_liquid_alpha = frame.wateralpha;
			...
			{
				extern byte	r_skypixels[6][SKYBOX_MAX_SIZE*SKYBOX_MAX_SIZE]; // Manoel Kasimier - hi-res skyboxes - edited

				pface = s->data;
				miplevel = 0;
				cacheblock = (byte *)(r_skypixels[pface->texinfo->texture->offsets[0]]); // Baker: Access violation here.
				// The map probably had a skybox, but it didn't load because wasn't PCX.
