...
void Z_Free (void *ptr)
{
	...
	if (!ptr)
		System_Error ("Z_Free: NULL pointer");

	block = (memblock_t *) ( (byte *)ptr - sizeof(memblock_t));
	if (block->id != ZONEID)
		System_Error ("Z_Free: freed a pointer without ZONEID");
	if (block->tag == 0)
		System_Error ("Z_Free: freed a freed pointer");
...
static void *Z_TagMalloc (int size, int tag)
{
	...
	if (!tag)
		System_Error ("Z_TagMalloc: tried to use a 0 tag");
...
static void Z_CheckHeap (void)
{
	...
	for (block = mainzone->blocklist.next ; ; block = block->next)
	{
		...
		if ( (byte *)block + block->size != (byte *)block->next)
			System_Error ("Z_CheckHeap: block size does not touch the next block");
		if ( block->next->prev != block)
			System_Error ("Z_CheckHeap: next block doesn't have proper back link");
		if (!block->tag && !block->next->tag)
			System_Error ("Z_CheckHeap: two consecutive free blocks");
	}
}
...
void *Z_Realloc(void *ptr, int size)
{
	...
	block = (memblock_t *) ((byte *) ptr - sizeof (memblock_t));
	if (block->id != ZONEID)
		System_Error ("Z_Realloc: realloced a pointer without ZONEID");
	if (block->tag == 0)
		System_Error ("Z_Realloc: realloced a freed pointer");
...
==============
*/
void Hunk_Check_f (lparse_t *line)
{
	hunk_t	*h;
	...
	for (h = (hunk_t *)hunk_base ; (byte *)h != hunk_base + hunk_low_used ; )
	{
		if (h->sentinal != HUNK_SENTINAL)
			System_Error ("Hunk_Check: trahsed sentinal");
		if (h->size < (int) sizeof(hunk_t) || h->size + (byte *)h - hunk_base > hunk_size)
			System_Error ("Hunk_Check: bad size");
...
===================
*/
void Hunk_Print_f (lparse_t *line)
{
	if (line->count == 2 && atoi(line->args[1]))
		Hunk_Print (true);
	else
	Hunk_Print (false);
}
...
cache_system_t *Cache_TryAlloc (int size, cbool nobottom);
...
void Cache_UnlinkLRU (cache_system_t *cs)
{
	if (!cs->lru_next || !cs->lru_prev)
		System_Error ("Cache_UnlinkLRU: NULL link");
...
{
	if (cs->lru_next || cs->lru_prev)
		System_Error ("Cache_MakeLRU: active link");
...
============
*/
void Cache_Flush (lparse_t *unused)
{
...
==============
*/
void Cache_Free (cache_user_t *c, cbool freetextures) //johnfitz -- added second argument
{
	cache_system_t	*cs;

	if (!c->data)
		System_Error ("Cache_Free: not allocated");
...
// find memory for it
	...
	// free the least recently used cahedat
		if (cache_head.lru_prev == &cache_head)
			System_Error ("Cache_Alloc: out of memory"); // not enough memory at all
...
{
	int zonesize = MEM_DYNAMIC_SIZE;
	int t;
	
	if ((t = COM_CheckParm("-heapsize")) && t + 1 < com_argc)
		host_parms.memsize = atoi (com_argv[t + 1]) * 1024;
	else if (isDedicated) host_parms.memsize = MEM_DEFAULT_DEDICATED; // Dedicated default
	else host_parms.memsize = MEM_DEFAULT_MEMORY;

	host_parms.membase = malloc (host_parms.memsize);

	if (!host_parms.membase)
		System_Error ("Not enough memory free; check disk space");
	...
	Cache_Init ();
	t = COM_CheckParm ("-zone");
	if (t)
	{
		if (t < com_argc-1)
			zonesize = atoi (com_argv[t + 1]) * 1024;
		else
			System_Error ("Memory_Init: you must specify a size in KB after -zone");
	}
