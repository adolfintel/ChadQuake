...
#include "quakedef.h" // Baker: Mods = skybox.  I suspect we need to do stuff here.
...
// draw or recurse further
	for (i = 0 ; i < 2 ; i++)
	{
		...
		// we're done with this branch if the node or leaf isn't in the PVS
			if (pn->visframe == cl.r_visframecount)
			{
				if (pn->contents < 0)
				{
...
// FIXME: use bounding-box-based frustum clipping info?
	...
	// draw the polygon
		...
		// outside the loop, and overflow checking must be done here
			...
			{
				System_Error ("no edges in bmodel");
			}
		}
	}
}
...
void Liquid_Think (cbool underwater)
{
...
void R_RenderWorld (void)
{
	...
	VectorCopy (r_origin, modelorg); // Baker r_origin set here. NO IT ISN'T.  VectorCopy is reversed.  Sheesh.  modelorg is being set.
	clmodel = currententity->model;
	r_pcurrentvertbase = clmodel->vertexes;

	R_PushDlights (NULL);// (); // R_PushDlights (cl.worldmodel->nodes);
	R_RecursiveWorldNode (clmodel->nodes, 15);
#ifdef WINQUAKE_SOFTWARE_SKYBOX
	if (skybox_name[0]) // Manoel Kasimier - skyboxes
