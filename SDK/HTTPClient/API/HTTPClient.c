...
#undef HTTP_CLIENT_DEFAULT_AGENT
#define HTTP_CLIENT_DEFAULT_AGENT g_user_agent
#endif // QUAKE_GAME
...
UINT32 HTTPClientSendRequest (HTTP_SESSION_HANDLE pSession,
							  ...
							  CHAR *pUserAgent,     //  [IN] Baker: User agent
                              ...
                              UINT32 nClientPort)   //  [IN] Client side port 0 for none
{
    ...
    do
    {
        ...
        if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,"User-Agent",10, pUserAgent /*HTTP_CLIENT_DEFAULT_AGENT*/,strlen(pUserAgent)))!= HTTP_CLIENT_SUCCESS)
        {
            break;
        }
...
UINT32 HTTPClientRecvResponse (HTTP_SESSION_HANDLE pSession,
                               UINT32 nTimeout, int *size, int *http_status_code) // [IN] Timeout for the operation
{
	...
	*http_status_code = pHTTPSession->HttpHeadersInfo.nHTTPStatus;
	*size = pHTTPSession->HttpHeadersInfo.nHTTPContentLength;	
    return nRetCode;
}
...
#ifdef _WIN32
    u_long           nNonBlocking    = 1;                // Baker: Stupid Winsock.
#else
