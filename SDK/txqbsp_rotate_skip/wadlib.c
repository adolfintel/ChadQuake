...
//
	...
	header.numlumps = tx_LittleLong(header.numlumps);
	header.infotableofs = tx_LittleLong(header.infotableofs);
...
//
	...
	{
		lump_p->filepos = tx_LittleLong(lump_p->filepos);
		lump_p->size = tx_LittleLong(lump_p->size);
	}
}
...
void NewWad (char *pathname, qboolean bigendien)
{
	...
	if (bigendien)
	{
		wadshort = tx_BigShort;
		wadlong = tx_BigLong;
	}
	else
	{
		wadshort = tx_LittleShort;
		wadlong = tx_LittleLong;
	}
