...
//
	LBM_P = LBMbuffer;
	if ( *(int *)LBMbuffer != tx_LittleLong(FORMID) )
	   Error ("No FORM ID at start of file!\n");

	LBM_P += 4;
	formlength = tx_BigLong( *(int *)LBM_P );
	LBM_P += 4;
	LBMEND_P = LBM_P + Align(formlength);

	formtype = tx_LittleLong(*(int *)LBM_P);
...
//
	...
	while (LBM_P < LBMEND_P)
	{
		...
		case BMHDID:
			...
			bmhd.w = tx_BigShort(bmhd.w);
			bmhd.h = tx_BigShort(bmhd.h);
			bmhd.x = tx_BigShort(bmhd.x);
			bmhd.y = tx_BigShort(bmhd.y);
			bmhd.pageWidth = tx_BigShort(bmhd.pageWidth);
			bmhd.pageHeight = tx_BigShort(bmhd.pageHeight);
			break;
...
//
	...
	*lbmptr++ = 'D';
	...
	basebmhd.w = tx_BigShort((short)width);
	basebmhd.h = tx_BigShort((short)height);
	basebmhd.nPlanes = tx_BigShort(8);
	basebmhd.xAspect = tx_BigShort(5);
	basebmhd.yAspect = tx_BigShort(6);
	basebmhd.pageWidth = tx_BigShort((short)width);
	basebmhd.pageHeight = tx_BigShort((short)height);

	memcpy (lbmptr,&basebmhd,sizeof(basebmhd));
	lbmptr += sizeof(basebmhd);

	length = lbmptr-(byte *)bmhdlength-4;
	*bmhdlength = tx_BigLong(length);
	if (length&1)
