#ifndef __ZIP_WIN_H__
#define __ZIP_WIN_H__
...
typedef int bool;
	...
	// Baker: DWORD = unsigned int 32
	// TCHAR = char
	// FILETIME = typedef struct { typedef struct _FILETIME {   DWORD dwLowDateTime; DWORD dwHighDateTime;} FILETIME, *PFILETIME, *LPFILETIME;
	// Change long to int32
	// HANDLE probably becomes a void *
...
extern "C"
#endif
HZIP CreateZipFile(const TCHAR *fn, const char *password);
HZIP CreateZipMemory(void *buf,unsigned int len, const char *password);
HZIP CreateZipHandle(HANDLE h, const char *password);
// CreateZip - call this to start the creation of a zip file.
// As the zip is being created, it will be stored somewhere:
// to a pipe:              CreateZipHandle(hpipe_write);
// in a file (by handle):  CreateZipHandle(hfile);
// in a file (by name):    CreateZipFile("c:\\test.zip");
// in memory:              CreateZipMemory(buf, len);
// or in pagefile memory:  CreateZipMemory(0, len);
...
extern "C"
#endif
ZRESULT ZipAddFile(HZIP hz,const TCHAR *dstzn, const TCHAR *fn);
ZRESULT ZipAddMemory(HZIP hz,const TCHAR *dstzn, void *src,unsigned int len);
ZRESULT ZipAddHandleNoLen(HZIP hz,const TCHAR *dstzn, HANDLE h);
ZRESULT ZipAddHandleLen(HZIP hz,const TCHAR *dstzn, HANDLE h, unsigned int len);
...
//     HZIP hz = CreateZipFile("c:\\simple1.zip",0);
...
//     HZIP hz = CreateZipMemory(0,100000, 0);
